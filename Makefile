...
DEBVERSION := 9.0
KERNELS_i386 := "linux-image-4.9.0-3-untangle-686-pae"
KERNELS_amd64 := "linux-image-4.9.0-3-untangle-amd64"
...
iso/%-image: debian-installer iso-conf repoint-stable
	...
	cd $$TMP_DIR ; \
	build-simple-cdd --keyring /usr/share/keyrings/untangle-archive-keyring.gpg --force-root --auto-profiles default,untangle,$(flavor) --profiles untangle,flavor,expert --debian-mirror http://package-server/public/$(REPOSITORY)/ --security-mirror http://package-server/public/$(REPOSITORY)/ --dist $(REPOSITORY) --require-optional-packages --extra-udeb-dist $(DISTRIBUTION) --do-mirror --verbose --logfile /tmp/simplecdd.log ; \
	cd $(ISOTOOLS_DIR) ; \
	rm -fr $$TMP_DIR ; \
